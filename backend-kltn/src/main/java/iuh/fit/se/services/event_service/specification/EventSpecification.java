...
public class EventSpecification {
    ...
    public static Specification<Event> isOfType(Class<? extends Event> eventType) {
        return (root, query, criteriaBuilder) -> {
            if (eventType == null) {
                return criteriaBuilder.conjunction();
            }
    ...
    public static Specification<Event> hasHostedUserId(String userId) {
        return (root, query, criteriaBuilder) -> 
            criteriaBuilder.equal(root.get("host").get("id"), userId);
    }
    
    public static Specification<Event> hasHostedUsername(String username) {
        return (root, query, criteriaBuilder) -> 
            criteriaBuilder.equal(root.get("host").get("username"), username);
    }
    
    public static Specification<Event> isNotDone() {
        return (root, query, criteriaBuilder) -> 
            criteriaBuilder.isFalse(root.get("isDone"));
    }
    
    public static Specification<Event> exceptOfType(Class<? extends Event> eventType) {
		return (root, query, criteriaBuilder) -> {
			if (eventType == null) {
				return criteriaBuilder.conjunction();
			}

...
public interface UserRepository
	...
	int incrementScoreForEventAttendees(@Param("event") Event event,
	...
	@Query("UPDATE User u SET u.contributionPoint = u.contributionPoint + :increment "
		...
		+ "WHERE e.event = :event )")
	int incrementScoreForEventOrganizers(@Param("event") Event event,
		@Param("increment") int increment);
	...
	int incrementScoreForContestWinners(@Param("contest") Contest contest,
		@Param("increment") int increment);
	...
	Page<User> findByUsernameContainingIgnoreCaseOrFullNameContainingIgnoreCaseOrId(
		String username, String fullName, String id,
		org.springframework.data.domain.Pageable pageable, Specification<User> spec);
}
